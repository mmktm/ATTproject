...
include 'connect.php'; //เชื่อมต่อDATABASE cloud
    ...
    $Time_Admin = date("H:i:s") ;
        ...
        if (!filter_var($Email, FILTER_VALIDATE_EMAIL) === false) {
            ...
            $sql_checkregisad = " SELECT * FROM `admin` 
                                WHERE Username = '$Username' OR Email = '$Email'" ;

            $result_checkregisad = $link->query($sql_checkregisad);

            if($result_checkregisad->num_rows == 1 ){
                // echo json_encode("error : username or email already exists"); 
                echo json_encode("error"); 

            }else{
                
                $sql_insertregisad = "INSERT INTO `admin` (Username, Passwordd , Email,Date_Admin,Time_Admin)
                                    VALUES ('$Username','$Passwordd','$Email','$Date_Admin','$Time_Admin')" ; //.md5 เข้ารหัส

                    $result_insertregisad = $link->query($sql_insertregisad);
                    if($result_insertregisad){
